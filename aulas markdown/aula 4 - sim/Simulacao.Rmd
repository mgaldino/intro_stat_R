...
# exercício para casa
1. Implemente uma função "my_rng"", que recebe um semente x, e o número de algorismos n, e retorna um vetor de número aleateorios de tamanho n.
...
Em parte pela sua conveniência matemática, e em parte por causa do teorema do limite central. O teorema do limite central diz, basicamente que a soma de variáveis aleatórias iid tendem a se distribuir como uma Normal.

Vamos então entender o TLC com simuação.
...
Nós podemos estar interessados em saber quão rápido z se aproxima da normal. Há várias formas de fazer isso.
Vou fazer aqui algo simples: fazer um teste de qui-quadrado de que as duas distribuições são iguais

## Simulação para ver quão rápido z se aproxima da normal, à medida que N cresce
```{r}

# preciso simular x, y e z para vários tamanhos de n
# vou usar um loop
vec_n <- (1:100)*100
vec_n
lista <- list()
...
for ( i in 1:length(vec_n)) {
  lista$x[[i]] <- runif(n=vec_n[i])
  lista$y[[i]] <- runif(n=vec_n[i])
  lista$w[[i]] <- runif(n=vec_n[i])
  z[[i]] <- lista$x[[i]] + lista$y[[i]] + lista$w[[i]]
}
